...
Produce the factorial of a number `a`(n!).
...
Produces the larger of two numbers, `a` and `b`.
...
Produces the smaller of two atoms, `a` and `b`.
...
Modulus: produce the remainder of dividing `a` by `b`.
