...
Produces 2 to the Nth power for some atom `a`.
...
Takes the base-2 logarithm of an atom `a`.
...
::
```
Assembles an atom from a list of length-value pairs `b`, where `p` is the length in bloqs of size `a`,
and `q` is an atomic value.

`a` is a [bloq](bloq) size.

`b` is a [list](list) of length value pairs.

    ~zod/try=> `@ub`(can 3 ~[[1 1]])
    0b1 
    ~zod/try=> `@ub`(can 0 ~[[1 255]])
    0b1
    ~zod/try=> `@ub`(can 1 ~[[1 2]])
    0b10
    ~zod/try=> `@ub`(can 1 ~[[1 3]])
    0b11
    ~zod/try=> `@ub`(can 1 ~[[1 4]])
    0b0
    ~zod/try=> `@ub`(can 1 ~[[2 4]])
    0b100
...
Concatenates two atoms, `b` and `c`, using bloq size `a`.
...
`c` is an [atom]().
    ...
    ~zod/try=> `@ub`(cat 1 1 0)
    0b1
    ~zod/try=> `@ub`(cat 2 1 0)
    0b1
    ~zod/try=> `@ub`(cat 4 1 0)
    0b1
...
Produces the tail of `d` after having right-shifted it by `b`. The length of the tail is determined by the block size `a`.
...
`c` is an [atom]().
    ...
    ~zod/try=> (cut 0 [2 1] 4)
    1
    ~zod/try=> (cut 3 [1 1] 256)
    1
    ~zod/try=> (cut 2 [1 1] 255)
    15
    ~zod/try=> (cut 1 [1 1] 255)
    3
    ~zod/try=> (cut 1 [1 2] 255)
    15
...
Produces the tail of `c`, whose length is determined by the number of bloqs `b` of size `a`.
...
`c` is an atom.
    ...
    ~zod/try=> `@t`(fil 3 5 %a)
    'aaaaa'
    ~zod/try=> `@t`(fil 5 10 %babe)
    'babebabebabebabebabebabebabebabebabebabe'
    ~zod/try=> `@tas`(fil 5 10 %babe)
    %babebabebabebabebabebabebabebabebabebabe
    ~zod/try=> `@tas`(fil 4 10 %babe)
    %ĆĆĆĆĆĆĆĆĆĆbe
...
Produces `c` left-shifted `b` times by block size `a`. 
...
Produces an atom representing the number of blocks of size `a` in `b`.
...
Concatenate a list of atoms `b` in accordance with a given blocksize `a`. Produces an atom.

    ~zod/try=> (rap 2 (limo [1 2 3 4 ~]))
    17.185
    ~zod/try=> (rap 1 (limo [1 2 3 4 ~]))
    313
...
Assemble an atom from a list of atoms `b` in accordance with bloq size `a`.

`a` is a bloq.

`b` is a list of atoms.

    ~zod/try=> (rep 0 (limo [1 2 3 4 ~]))
    5
    ~zod/try=> (rep 1 (limo [1 2 3 4 ~]))
    57
    ~zod/try=> (rep 0 (limo [1 0 0 ~]))
    1
    ~zod/try=> (rep 0 (limo [1 0 0 0 ~]))
    1
...
Produces an atomic list of the bits of an atom `a` according to the block size `a`
...
Switches little ending to big and vice versa: produces the reversed block order
of a number `b` in accordance with bloq size `a`.
...
Get byte from right-hand syllable.
...
Get byte from left-hand phonetic syllable
...
Selects left-hand phonetic syllable from 'sis'.
...
Binary block modulo math engine. Defaults to one single operations
...
Inverse of `yall`, computes number of days A.D. from y/m/d date.
...
Determines whether a year contains an ISO 8601 leap week.
