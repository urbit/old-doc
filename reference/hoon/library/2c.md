...
::
```
Produces an atom from a list of length-value pairs `b`, where `p` is the length in bloqs of size `a`,
...
Produces `c` blocks of size `a` that are `b` blocks from the end of `d`.
...
Produces an atom by taking the last `b` bloqs of size `a` from `c`.
...
Produces an atom by assembling a list of atoms `b` in accordance with bloq size `a`.
...
Switches little ending to big and vice versa: produces an atom by reversing the block order
of `b` using block size `a`.
...
size 5 off the product's end, producing atom.
